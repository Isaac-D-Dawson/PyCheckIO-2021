...
def words_order(text: str, words: list) -> bool:
    '''
        takes a string and a list of words. returns true if the words appear in order in the string. Returns False otherwise.
    '''
    ...
    for i in words:
        if i not in text:
            return False #We can bypass all further checks, as it fails here
            
    if len(words) > len(set(words)):
        return False
    
    if len(words) == 1 and words[0] in text:
        return True #Same thing, we can bypass all further checks if there's only one word.
    
    inval = []
    
    for i in words:
        inval.append(text.index(i))
    for i in range(0, len(inval)-1):
