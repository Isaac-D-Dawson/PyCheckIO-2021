...
def sun_angle(time:str) -> [str, float]:
	...
	#Datetime could probably do this better, but this was easier
    hour = int(time[0:2])	#set the hour as the hours amount in the time
    mins = int(time[3:])	#set the minutes to the minutes amount.
    
    if 6 > hour or hour > 18 or (hour == 18 and mins > 00):	#ensures the sun is above the horizon.
        return "I don't see the sun!"
    
    outval  = (hour-6) * 15	#calculates the rough angle using the hours...
    outval += (mins/4)		#and improves that with the minuites. Had to work out the math manually, took a while
